...
class GhostModule(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1, relu=True):
        ...
        self.primary_conv = nn.Sequential(
            ...
            nn.LeakyReLU(0.2) if relu else nn.Sequential(),
        )
...
class GeneratorGhostConv(nn.Module):

    def __init__(self, ratio=1, keyPoints=68, dropOutRate=0.1, temperature=1., imageSize=80):
        ...
        self.blockE1 = nn.Sequential(
            GhostModule(3, self.featureMaps[0]),
#             self.activation
        )
...
#             self.activation,
    ...
    def forward(self, x):
        ...
        out = self.blockT(torch.cat((D1, E1), dim=1)) # output shape: (b, x, 80, 80)
    ...
    def KLDivergenceBetween2LaplacianDistribution(self, miu_1, lambda_1, miu_2, lambda_2, epsilon=1e-9, sigma=0.):
...
# inputdata = torch.randn((1, 3, 80, 80))
# a = Generator()
# out = a(inputdata)
# print(out.size())
